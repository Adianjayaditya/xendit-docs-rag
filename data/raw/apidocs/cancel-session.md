---
url: https://docs.xendit.co/apidocs/cancel-session
...
- Published on Dec 4, 2024
 ...
 [Prev](/apidocs/get-session "Get the status of a session")  [Next](/apidocs/webhook-notification-sent-defined-webhook-url-updates-payment-session "Webhook notification that will be sent to your defined webhook url for updates to payment session status.") 

Post

/sessions/{session\_id}/cancel

Cancels an ACTIVE Session.
The Xendit Hosted Checkout URL will be invalid.
...
Error response for POST /sessions//cancel. Possible HTTP status codes: 401, 404, 422.
...
```
{
  ...
  "message": "Session status is COMPLETED, expecting ACTIVE"
}
```
...
- `INVALID_SESSION_STATUS` - The session is not in ACTIVE status. Only ACTIVE sessions can be canceled.

Valid values[
"INVALID\_API\_KEY",
"NOT\_FOUND",
"INVALID\_SESSION\_STATUS"
]
